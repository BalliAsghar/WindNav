...
final class LoggerTests: XCTestCase {
    ...
    func testErrorLevelSuppressesInfoAndDebug() {
        let sink = SinkBox()
        Logger._setTestSink { line in sink.append(line) }
        Logger._setTestNowProvider { Date(timeIntervalSince1970: 0) }
    ...
    func testDebugLevelAllowsDebugInfoError() {
        let sink = SinkBox()
        Logger._setTestSink { line in sink.append(line) }
        Logger._setTestNowProvider { Date(timeIntervalSince1970: 0) }
        Logger.configure(level: .debug, colorMode: .never)
        ...
        XCTAssertTrue(lines[0].contains("Debug message"))
        XCTAssertTrue(lines[1].contains("Info message"))
        XCTAssertTrue(lines[2].contains("Error message"))
    }

    func testTimestampIsHHMMSS() {
        let sink = SinkBox()
        Logger._setTestSink { line in sink.append(line) }
        Logger._setTestNowProvider { Date(timeIntervalSince1970: 0) }
        Logger.configure(level: .info, colorMode: .never)
        ...
        guard let line = sink.lines().first else {
            XCTFail("Expected one log line")
            return
        }
        guard let close = line.firstIndex(of: "]"), line.hasPrefix("[") else {
            XCTFail("Expected bracketed timestamp prefix")
            return
        }
